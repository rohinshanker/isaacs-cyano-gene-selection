...
async (page) => {
  ...
  const parity = await page.evaluate(async () => {
    ...
    const report = await performanceClient.run({ dataset, ids: ten, map: { TCG: 'AGC', TCA: 'AGT', TAG: 'TAA' } });
    const elapsedMs = performance.now() - start;
    clearInterval(ticker);
    const second = await performanceClient.run({ dataset, ids: ten, map: { TCG: 'AGC', TCA: 'AGT', TAG: 'TAA' } });
    ...
    return { cases: reference.cases.length, worst, elapsedMs, ticks, maxTickGapMs: Math.max(...gaps), cacheHits, errors };
  });
