...
export const ROSETTA_FORMATS = Object.freeze(['sequence', 'fasta', 'a3m', 'a2m', 'stockholm']);
...
export function handoffSequence({ gene, table, map = {}, form = 'wild-type', region = 'start', start, end }) {
  ...
  if (region !== 'range') throw new Error('Choose a supported RNA region.');
...
export function dotBracketToCt(sequence, structure, title = 'RNA') {
  ...
  const lines = [`${sequence.length}\t${title}`];
...
export function parseCt(text) {
  ...
  const structure = pairs.map((mate, index) => mate === 0 ? '.' : mate > index + 1 ? '(' : ')').join('');
  return { sequence: sequence.join(''), structure };
}

export function writeHandoffFormats({ sequence, header, structure = null }) {
  ...
  if (structure !== null) {
    ...
    files.ct = dotBracketToCt(sequence, structure, safeHeader);
  }
  return files;
}
