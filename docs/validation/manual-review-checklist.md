...
| GO IEA essentiality context | 31 loci without a determinate PCC call show a TypeSafe Jev GO IEA core-process context tier. Every disagreement with a product name, reviewed category, or PCC call is noted. See [`go-iea-essentiality-context.md`](go-iea-essentiality-context.md). | Treat the tier as computational context only. Review every discrepancy note on a proposed target before trusting either source. |
